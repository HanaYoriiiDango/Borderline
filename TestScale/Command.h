...
void StartGame() {
    ...
    while (Hero.life && !end_game) {
        ...
        if (temp == "Go") CommandGo(); // выводит все доступные миры для игрока и позволяет перемещаться по тем мирам что доступы игроку  
        if (temp == "Loc") cout << "Current loc: " << Hero.current_loc << endl; // выводим игроку его текущее положение
        if (temp == "Start") Startdialog(); // начинаем диалог с персонажем
    }
}
...
3) Адаптирвовать код для обновленной инициалиазации 
4)
